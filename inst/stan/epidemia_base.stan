...
#include /tparameters/tparameters_glm.stan
...

  {
    ...
    for (proc in 1:ac_nproc) { // this treats ac terms as random walks for now (to be extended to AR(p))
        // non-centred RW: rescale unit-scale increments before accumulating
        ac_noise[i:(i+ac_ntime[proc]-1)] = ac_scale[proc] * ac_noise_raw[i:(i+ac_ntime[proc]-1)];
    ...
    for (proc in 1:obs_ac_nproc) {
        obs_ac_noise[i:(i+obs_ac_ntime[proc]-1)] = obs_ac_scale[proc] * obs_ac_noise_raw[i:(i+obs_ac_ntime[proc]-1)];
