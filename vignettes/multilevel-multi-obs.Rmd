...
@Flaxman2020. To keep the example quick we restrict it to six countries.

Six rather than three, because the number of groups is not just a matter of
runtime here. A partially pooled slope asks the data to estimate a *variance*
across groups, and with three groups that variance is barely identified: the
global effect $\beta$ and the deviations $b^{(m)}$ can trade off against each
other almost freely, which shows up as a ridge the sampler has to crawl along.
Fitting the same model to three countries leaves divergent transitions and a
tail effective sample size in the tens; at six they disappear, and the fit is
no slower despite twice the data, because the sampler stops fighting the
geometry.
...
groups <- c("Austria", "Belgium", "Denmark", "Germany", "Italy", "Switzerland")
...
```
## Chain 2 finished in 148.4 seconds.
## Chain 4 finished in 154.6 seconds.
## Chain 3 finished in 157.3 seconds.
## Chain 1 finished in 157.8 seconds.
## 
## All 4 chains finished successfully.
## Mean chain execution time: 154.5 seconds.
## Total execution time: 157.8 seconds.
```
...
```
##                5%   50%   95%
## Austria     -3.93 -3.53 -3.21
## Belgium     -2.84 -2.66 -2.49
## Denmark     -1.85 -1.67 -1.49
## Germany     -2.63 -2.51 -2.38
## Italy       -2.51 -2.40 -2.29
## Switzerland -2.48 -2.36 -2.24
```
...
## [3] "cases|countryDenmark"        "cases|countryGermany"       
## [5] "cases|countryItaly"          "cases|countrySwitzerland"   
## [7] "cases|reciprocal dispersion"
```
...
```
##     Austria     Belgium     Denmark     Germany       Italy Switzerland 
##       0.253       0.066       0.183       0.259       0.055       0.191
```
...
```
##     Austria     Belgium     Denmark     Germany       Italy Switzerland 
##       0.247       0.062       0.165       0.248       0.052       0.188
```
...
making a defensible epidemiological claim. Six countries is still a thin basis
