...
**Estimation uses MCMC.** An earlier R vignette fitted with Variational
...
`partial-pooling.py` still uses. Datasets are
...
```bash
uv run jupyter lab notebooks/b117.ipynb                    # or open the .py directly
```
...
> several minutes for a good `multilevel-multi-obs` fit; reduce `draws`/`tune`, or the
